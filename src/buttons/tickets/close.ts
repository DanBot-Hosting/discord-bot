            ...
            collector.on("collect", async i => {
                if(i.customId === `confirm-${interaction.id}`) {
                    ...
                    messages.forEach(m => {
                        ...
                        const author = m.author.tag.endsWith("#0") ? m.author.username : m.author.tag;
                    ...
                    const transcriptFile = new Discord.AttachmentBuilder(Buffer.from(transcript), { name: `transcript-${user}-${Date.now()}.txt` });
